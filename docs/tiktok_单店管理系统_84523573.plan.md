...
- 订单同步与订单行持久化的正常详情场景，以及缺字段保留、明确空列表清空、非空列表替换语义。
- Collector 从任务到 Core 草稿的本地跨进程边界。
- 翻译 Provider 纯边界、管理员 Session/CSRF/幂等/审计贯通的离线 HTTP E2E、`ToolsView` 成功/失败交互、利润、调度和审计。
...
4. 订单领域对象已显式携带 `line_items/items` presence；字段缺失时更新摘要并保留已存行，明确空列表时清空，非空列表时替换。
5. 翻译已补配置成功和失败关闭的离线 HTTP E2E，`ToolsView` 已补成功/失败组件交互测试；受控 transport 结果不等于 live Azure 验收。

**仍待关闭**

1. `.env.example` 当前只包含妙手变量，无法指导完整 Core/Collector/TikTok/Azure 配置。
...
**状态：已完成（2026-08-07，离线/窄范围验证）。** 订单语义与工具链专项回归已通过；真实 Azure 验收及 P0 完整质量门槛仍未完成。

- 不完整订单详情不再清空已有行。
- 明确空行详情仍可清空已存行，非空行可完整替换。
- 翻译 HTTP E2E 已贯通 Session、CSRF、幂等、受控 Provider transport 与审计；成功/失败均有稳定边界测试。
- ToolsView 成功/失败交互测试已通过，前端 typecheck 通过。

**Checkpoint P0-C：配置与证据基线**

**状态：已完成（2026-08-07，配置静态核对与脱敏阻塞证据）。** `.env.example` 与最小 live-check 格式已有离线证据；妙手真实只读请求因进程环境未显式启用而未执行。

- `.env.example` 已覆盖 29 个实际读取的运行时变量，Secret/Token/Key 均为空，只保留公开安全默认值。
- 妙手只读证据产物已记录 `BLOCKED / MIAOSHOU_PROVIDER_DISABLED`；没有请求网络或调用写接口。
- 脱敏格式严格限制字段，店铺身份只可写为不可逆短指纹，runner 与 JSON 原子写出职责分离。

**完成门槛**

- Ruff、后端完整 pytest、前端 typecheck/test/build 全部通过。
- 无敏感信息进入 Git Diff、测试输出和进度文档。
- 本轮因安全约束未运行缓存类测试、完整 pytest 或前端 build，故 `p0-trustworthy-baseline` 聚合状态继续保持 `in_progress`。
...
进入 **P0 安全质量门槛收口/评估**：在不运行被禁止的缓存类测试、不创建构建缓存且不执行目录清理的前提下，先审计剩余完整 pytest、前端全量 test/typecheck/build 门槛是否可安全执行；无法安全执行的项目继续明确记录为未完成。`p0-trustworthy-baseline` 聚合状态保持 `in_progress`，不得直接跳入 P1 或把妙手阻塞产物表述为真实查询通过。
